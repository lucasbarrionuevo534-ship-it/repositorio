...
function aplicarFiltros() {
    ...
    if (productosFiltrados.length === 0) {
        ...
        contenedorCatalogo.innerHTML = productosFiltrados
            .map(producto => crearTarjetaProductoHTML(producto, favoritosIds.includes(producto.id)))
            .join("");
    }
}
...
contenedorCatalogo.addEventListener("click", (evento) => {
    ...
    console.log("Elemento cliqueado:", evento.target, "Botón detectado:", botonFav);

    if (!botonFav) return;

    const id = Number(botonFav.dataset.id);
    console.log("ID procesado:", id, typeof id);
